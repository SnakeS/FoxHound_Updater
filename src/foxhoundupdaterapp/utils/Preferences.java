package foxhoundupdaterapp.utils;
...
import foxhoundupdaterapp.customTypes.FullExtraList;
import foxhoundupdaterapp.customTypes.ExtraInfo;
import foxhoundupdaterapp.customTypes.ExtraList;
import foxhoundupdaterapp.customization.Customization;
import foxhoundupdaterapp.database.DbAdapter;
import foxhoundupdaterapp.misc.Constants;
import foxhoundupdaterapp.misc.Log;
import foxhoundupdaterapp.ui.R;
...
public class Preferences {
    ...
    public String getFeaturedExtrasURL() {
        temp = mRes.getString(R.string.conf_featured_extras_url);
        if (showDebugOutput) Log.d(TAG, "FeaturedExtrasURL: " + temp);
        return temp;
    }
    ...
    //Extras
    public boolean showAllExtraUpdates() {
        tempbool = mPrefs.getBoolean(mRes.getString(R.string.PREF_DISPLAY_OLDER_EXTRA_VERSIONS), Boolean.valueOf(mRes.getString(R.string.PREF_DISPLAY_OLDER_EXTRA_VERSIONS_DEF_VALUE)));
        if (showDebugOutput) Log.d(TAG, "Display All Extra Updates: " + tempbool);
        return tempbool;
    }

    public boolean showExperimentalExtraUpdates() {
        tempbool = mPrefs.getBoolean(mRes.getString(R.string.PREF_DISPLAY_EXPERIMENTAL_EXTRA_VERSIONS), Boolean.valueOf(mRes.getString(R.string.PREF_DISPLAY_EXPERIMENTAL_EXTRA_VERSIONS_DEF_VALUE)));
        if (showDebugOutput) Log.d(TAG, "Display Experimental Extra Updates: " + tempbool);
        return tempbool;
    }

    public LinkedList<ExtraList> getExtraUpdateUrls() {
        DbAdapter extraListDb = new DbAdapter(context, showDebugOutput);
        if (showDebugOutput) Log.d(TAG, "Opening Database");
        FullExtraList fullExtraList = new FullExtraList();
        //Get the actual ExtraList from the Database
        Cursor extraListCursor = null;
        try {
            extraListDb.open();
	        ...
	        extraListDb.close();
	        return fullExtraList.returnFullExtraList();
        } finally {
        	if (extraListCursor != null && !extraListCursor.isClosed())
        		extraListCursor.close();
        }
    }

    public String getExtraFile() {
        temp = mPrefs.getString(mRes.getString(R.string.PREF_EXTRAS_EXTRA_FILE), mRes.getString(R.string.conf_extra_version_file_def));
        if (showDebugOutput) Log.d(TAG, "ExtraFile: " + temp);
        return temp;
    }

    public void setExtraFile(String path) {
        Editor editor = mPrefs.edit();
        editor.putString(mRes.getString(R.string.PREF_EXTRAS_EXTRA_FILE), path);
        if (!editor.commit()) Log.e(TAG, "Unable to write Extra File Path");
    }

    public boolean ExtraUpdateUrlSet() {
        DbAdapter extraListDb = new DbAdapter(context, showDebugOutput);
        if (showDebugOutput) Log.d(TAG, "Opening Database");
        extraListDb.open();
        long count = extraListDb.getExtraCount();
        extraListDb.close();
        return count > 0;
    }
    ...
    public ExtraInfo getExtraInformations() {
        File f = new File(getExtraFile());
        if (f.exists() && f.canRead()) {
            ...
            if (showDebugOutput) Log.d(TAG, "No Extra File found. Using Wildcard for Extra Updates instead");
        }
        return null;
    }
}
